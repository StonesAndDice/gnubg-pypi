...
CubeInfoToPy(const cubeinfo * pci)
{
    return Py_BuildValue("{s:i,s:i,s:i,s:i,s:(i,i),"
                         ...
                         "gammonprice",
                         pci->arGammonPrice[0],
                         pci->arGammonPrice[2], pci->arGammonPrice[1], pci->arGammonPrice[3],
                         "bgv", pci->bgv);
}
...
PyToCubeInfo(PyObject * p, cubeinfo * pci)
{
    ...
    while (PyDict_Next(p, &iPos, &pyKey, &pyValue)) {
        ...
        if (iKey < 0) {
            PyErr_SetString(PyExc_ValueError, "invalid key in cubeinfo (see gnubg.cubeinfo() for an example)");
            return -1;
        }
        ...
        case 0: case 1: case 2: case 3: case 4: case 5: case 6: case 7:
            if (!PyLong_Check(pyValue)) {
                PyErr_SetString(PyExc_ValueError, "invalid value in cubeinfo (see gnubg.cubeinfo() for an example)");
                return -1;
            }
...
PyToPosInfo(PyObject * p, posinfo * ppi)
{
    ...
    while (PyDict_Next(p, &iPos, &pyKey, &pyValue)) {
        ...
        if (iKey < 0) {
            PyErr_SetString(PyExc_ValueError, "invalid key in posinfo (see gnubg.posinfo() for an example)");
            return -1;
        }
        ...
        case 0: case 1: case 2:
            if (!PyLong_Check(pyValue)) {
                PyErr_SetString(PyExc_ValueError, "invalid value in posinfo (see gnubg.posinfo() for an example)");
                return -1;
            }
            *((int *) apv[iKey]) = (int) PyLong_AsLong(pyValue);
            break;
 ...
 */
static PyObject *
METRow(const float ar[MAXSCORE], const int n)
{
 ...
 */
static PyObject *
METPre(const float aar[MAXSCORE][MAXSCORE], const int n)
{
    PyObject *pyList = PyList_New(n);
    if (!pyList)
        return NULL;
...
/*
 * Ported from gnubgmodule.c: MoveFilterToPy, MoveFiltersToPy, PyToMoveFilter, PyToMoveFilters
 * For getevalhintfilter / setevalhintfilter.
 */
static PyObject *
MoveFilterToPy(int ply, int level, const movefilter * pmf)
{
    return Py_BuildValue("{s:i,s:i,s:i,s:i,s:f}",
                         "ply", ply, "level", level,
                         "acceptmoves", pmf->Accept, "extramoves", pmf->Extra, "threshold", (double) pmf->Threshold);
}
...
PyToMoveFilter(PyObject * p, movefilter * pmf, int *ply, int *level)
{
    ...
    while (PyDict_Next(p, &iPos, &pyKey, &pyValue)) {
        ...
        Py_DECREF(keyBytes);
        if (i < 0) {
            PyErr_SetString(PyExc_ValueError, "invalid key in movefilter (see gnubg.getevalhintfilter() for an example)");
            return -1;
        }
        switch (i) {
        case 0: case 1: case 2: case 3:
            if (!PyLong_Check(pyValue)) {
                PyErr_SetString(PyExc_ValueError, "invalid value in movefilter (see gnubg.getevalhintfilter() for an example)");
                return -1;
            }
...
RolloutContextToPy(const rolloutcontext * rc)
{
    return Py_BuildValue("{s:i,s:i,s:i,s:i,s:i,s:i,s:i,s:i,s:i,s:i,"
                        ...
                        "std-limit", (double) rc->rStdLimit,
                        "jsd-limit", (double) rc->rJsdLimit);
}

/*
 * Exposed as: gnubg.rolloutcontext([fCubeful, fVarRedn, ...])
 * Make a rollout context dict from optional 16 ints + 2 floats; defaults when omitted.
 */
static PyObject *
PythonRolloutContext(PyObject * self, PyObject * args)
{
    rolloutcontext rc;
    int fCubeful = 1, fVarRedn = 1, fInitial = 0, fRotate = 1, fLateEvals = 0, fDoTruncate = 0;
    int nTruncate = 10, fTruncBearoff2 = 1, fTruncBearoffOS = 1, fStopOnSTD = 0;
    int nTrials = 1296, nSeed = 0, nMinimumGames = 324, fStopOnJsd = 0, nLate = 5, nMinimumJsdGames = 324;
    float rStdLimit = 0.01f, rJsdLimit = 2.33f;

    (void) self;
    memset(&rc, 0, sizeof(rc));
    if (!PyArg_ParseTuple(args, "|iiiiiiiiiiiiiiiiff",
                          &fCubeful, &fVarRedn, &fInitial, &fRotate, &fLateEvals, &fDoTruncate,
                          &nTruncate, &fTruncBearoff2, &fTruncBearoffOS, &fStopOnSTD,
                          &nTrials, &nSeed, &nMinimumGames, &fStopOnJsd, &nLate, &nMinimumJsdGames,
                          &rStdLimit, &rJsdLimit))
        return NULL;
...
/* Convert Python (d0, d1) to int anDice[2] (values 1-6). Returns 1 on success, 0 on failure. */
static int
...
static int
PyToMoveFilters(PyObject * p, movefilter aamf[MAX_FILTER_PLIES][MAX_FILTER_PLIES])
{
    ...
    if (!PySequence_Check(p)) {
        PyErr_SetString(PyExc_ValueError, "invalid movefilter list (see gnubg.getevalhintfilter() for an example)");
        return -1;
    }
    ...
    for (Py_ssize_t entry = 0; entry < iSeqLen; ++entry) {
        ...
        if (PyToMoveFilter(pObj, &newFilter, &iPly, &iLevel) != 0) {
            Py_DECREF(pObj);
            PyErr_SetString(PyExc_ValueError, "invalid movefilter in list (see gnubg.getevalhintfilter() for an example)");
            return -1;
        }
        Py_DECREF(pObj);
...
PythonCubeInfo(PyObject * self, PyObject * args)
{
    ...
    if (SetCubeInfo(&ci, nCube, fCubeOwner, fMove, nMatchTo, anScore,
                    fCrawford, fJacobyRule, fBeavers, bgv)) {
 ...
 */
static PyObject *
PythonPosInfo(PyObject * self, PyObject * args)
{
    posinfo pi;
    ...
    if (ms.anDice[0] >= 0 && ms.anDice[0] <= 6 && 
        ms.anDice[1] >= 0 && ms.anDice[1] <= 6) {
...
PythonEvalContext(PyObject * self, PyObject * args)
{
    ...
    if (!PyArg_ParseTuple(args, "|iiiif:evalcontext",
...
/* Convert Python dict (from evalcontext()) to evalcontext. Returns 0 on success, -1 on error. */
static int
PyToEvalContext(PyObject * p, evalcontext * pec)
{
    ...
    if (!PyDict_Check(p)) {
        PyErr_SetString(PyExc_TypeError, "evalcontext must be a dict (see gnubg.evalcontext())");
        return -1;
    }
    ...
    while (PyDict_Next(p, &iPos, &pyKey, &pyValue)) {
        ...
        if (!pchKey) { Py_DECREF(utf8); return -1; }

        for (int i = 0; aszKeys[i]; ++i)
            if (strcmp(aszKeys[i], pchKey) == 0) { iKey = i; break; }
        Py_DECREF(utf8);

        if (iKey < 0) {
            PyErr_SetString(PyExc_ValueError, "invalid key in evalcontext (see gnubg.evalcontext())");
            return -1;
        }
        ...
        case 3: /* prune */
            if (!PyLong_Check(pyValue)) {
                PyErr_SetString(PyExc_ValueError, "invalid value in evalcontext (see gnubg.evalcontext())");
                return -1;
            }
            {
                long i = PyLong_AsLong(pyValue);
                if (iKey == 0) pec->fCubeful = i ? 1 : 0;
                else if (iKey == 1) pec->nPlies = (i < 8) ? (unsigned int) i : 7;
                else if (iKey == 2) pec->fDeterministic = i ? 1 : 0;
                else pec->fUsePrune = i ? 1 : 0;
            }
            break;
        case 4: /* noise */
            if (!PyFloat_Check(pyValue)) {
 ...
 * Evaluate position; returns tuple of 6 floats (win, wingammon, winbackgammon, losegammon, losebackgammon, equity).
 */
static PyObject *
PythonEvaluate(PyObject * self, PyObject * args)
{
    ...
    if (!PyArg_ParseTuple(args, "|OOO:evaluate", &pyBoard, &pyCubeInfo, &pyEvalContext))
        return NULL;
    ...
    return Py_BuildValue("(ffffff)",
                         (double) arOutput[0], (double) arOutput[1], (double) arOutput[2],
                         (double) arOutput[3], (double) arOutput[4], (double) arOutput[5]);
}

/*
 * Exposed as: gnubg.findbestmove([board], [cubeinfo], [evalcontext], [dice], [movefilters])
 * Find best move for the given dice; returns tuple of (from, to, ...) 1-based, or empty tuple if no move.
 */
static PyObject *
PythonFindBestMove(PyObject * self, PyObject * args)
{
    PyObject *pyBoard = NULL;
    PyObject *pyCubeInfo = NULL;
    PyObject *pyEvalContext = NULL;
    ...
    if (!PyArg_ParseTuple(args, "|OOOOO:findbestmove",
                          &pyBoard, &pyCubeInfo, &pyEvalContext, &pyDice, &pyMoveFilters))
        return NULL;

    if (pyDice && !PyToDice(pyDice, anDice)) {
        PyErr_SetString(PyExc_TypeError, "dice must be a sequence of 2 integers (1-6)");
        return NULL;
    }
    if (!pyDice && ms.gs == GAME_PLAYING && ms.anDice[0] >= 1 && ms.anDice[0] <= 6 && ms.anDice[1] >= 1 && ms.anDice[1] <= 6) {
        anDice[0] = ms.anDice[0];
        anDice[1] = ms.anDice[1];
    }
    if (anDice[0] < 1 || anDice[0] > 6 || anDice[1] < 1 || anDice[1] > 6) {
        PyErr_SetString(PyExc_ValueError, "dice required: provide (die1, die2) with values 1-6");
        return NULL;
    }
 ...
 */
static PyObject *
PythonClassifyPosition(PyObject * self, PyObject * args)
{
    PyObject *pyBoard = NULL;
    TanBoard anBoard;
 ...
 */
static PyObject *
PythonLuckRating(PyObject * self, PyObject * args)
{
    float r;
 ...
 */
static PyObject *
PythonErrorRating(PyObject * self, PyObject * args)
{
    float r;
 ...
 */
static PyObject *
PythonParseMove(PyObject * self, PyObject * args)
{
    PyObject *pyMoves;
    char *pch;
    char *sz;
 ...
 */
static PyObject *
PythonMoveTuple2String(PyObject * self, PyObject * args)
{
    PyObject *pyBoard = NULL;
 ...
 */
static PyObject *
PythonMatchID(PyObject * self, PyObject * args)
{
    PyObject *pyCubeInfo = NULL;
    ...
    if (pyCubeInfo && !pyPosInfo) {
        PyErr_SetString(PyExc_TypeError, "matchid: cubeinfo requires posinfo (see gnubg.matchid())");
        return NULL;
    }
 ...
 */
static PyObject *
PythonGnubgID(PyObject * self, PyObject * args)
{
    PyObject *pyBoard = NULL;
    PyObject *pyCubeInfo = NULL;
    PyObject *pyPosInfo = NULL;
    ...
    if (pyBoard && (!pyPosInfo || !pyCubeInfo)) {
        PyErr_SetString(PyExc_TypeError, "gnubgid requires 0 or exactly 3 arguments (board, cubeinfo, posinfo). See gnubg.board(), gnubg.cubeinfo(), gnubg.posinfo().");
        return NULL;
    }
    if (pyBoard && !PyToBoard(pyBoard, anBoard)) {
        PyErr_SetString(PyExc_TypeError, "Invalid board format");
 ...
 */
static PyObject *
PythonDiceRolls(PyObject * self, PyObject * args)
{
    long n;
 ...
 */
static PyObject *
PythonMatchChecksum(PyObject * self, PyObject * args)
{
...
PythonPositionBearoff(PyObject * self, PyObject * args)
{
    ...
    return PyLong_FromLong((long) PositionBearoff(anBoard[0], (unsigned int) nPoints, (unsigned int) nChequers));
}
...
PythonPositionFromBearoff(PyObject * self, PyObject * args)
{
    ...
    unsigned int n = Combination((unsigned int) (nChequers + nPoints), (unsigned int) nPoints);
    if (iPos >= n) {
        PyErr_SetString(PyExc_ValueError, "invalid position number");
        return NULL;
    }
    PositionFromBearoff(anBoard[0], iPos, (unsigned int) nPoints, (unsigned int) nChequers);
 ...
 */
static PyObject *
PythonEq2mwcStdErr(PyObject * self, PyObject * args)
{
    PyObject *pyCubeInfo = NULL;
    float r = 0.0f;
    cubeinfo ci;
 ...
 */
static PyObject *
PythonMwc2eqStdErr(PyObject * self, PyObject * args)
{
    PyObject *pyCubeInfo = NULL;
    float r = 0.0f;
    cubeinfo ci;
...
static int PythonHint_Callback(procrecorddata * pr)
{
    ...
    const matchstate *pms = (const matchstate *) pr->avOutputData[PROCREC_HINT_ARGOUT_MATCHSTATE];
    const movelist *pml = (const movelist *) pr->avOutputData[PROCREC_HINT_ARGOUT_MOVELIST];
    ...
    switch (pes->et) {
    case EVAL_NONE:
        hintdict = Py_BuildValue("{s:i,s:s,s:f,s:f}", "movenum", index + 1, "type", "none", "equity", (double) rEq, "eqdiff", (double) rEqDiff);
        break;
    case EVAL_EVAL: {
        const move *pmi = &pml->amMoves[index];
        details = Py_BuildValue("{s:(fffff),s:f}", "probs",
                                (double) pmi->arEvalMove[0], (double) pmi->arEvalMove[1],
                                (double) pmi->arEvalMove[2], (double) pmi->arEvalMove[3], (double) pmi->arEvalMove[4],
        ...
        hintdict = Py_BuildValue("{s:i,s:s,s:s,s:f,s:f,s:N,s:N}", "movenum", index + 1, "type", "eval", "move", szMove,
                                "equity", (double) rEq, "eqdiff", (double) rEqDiff, "context", ctxdict, "details", details);
        break;
    }
    case EVAL_ROLLOUT: {
        const move *pmi = &pml->amMoves[index];
        ...
        details = Py_BuildValue("{s:(fffff),s:(fffff),s:f,s:f,s:f,s:f,s:i,s:f}",
                                "probs", (double) p[0], (double) p[1], (double) p[2], (double) p[3], (double) p[4],
                                "probs-std", (double) s[0], (double) s[1], (double) s[2], (double) s[3], (double) s[4],
                                "match-eq", (double) p[OUTPUT_EQUITY],
                                "cubeful-eq", (double) p[OUTPUT_CUBEFUL_EQUITY],
                                "score", (double) pmi->rScore, "score2", (double) pmi->rScore2,
                                "trials", (int) pes->rc.nGamesDone,
    ...
    default:
        hintdict = Py_BuildValue("{s:i,s:s,s:i}", "movenum", index + 1, "type", "unknown", "typenum", (int) pes->et);
        break;
    }
    if (hintdict) {
 ...
 */
static PyObject *
PythonHint(PyObject * self, PyObject * args)
{
    int nMaxMoves = -1;
    char szNumber[11];
    ...
    if (!ms.anDice[0] && !ms.fDoubled && !ms.fResigned) {
        PyErr_SetString(PyExc_RuntimeError, "Hints for cube actions not yet implemented");
        return NULL;
    }
    if (ms.fResigned) {
        PyErr_SetString(PyExc_RuntimeError, "Hints for resignations not yet implemented");
        return NULL;
    }
    if (ms.fDoubled) {
        PyErr_SetString(PyExc_RuntimeError, "Hints for take actions not yet implemented");
        return NULL;
    }
    if (!ms.anDice[0]) {
    ...
    prochint.avInputData[PROCREC_HINT_ARGIN_MAXMOVES] = (void *) (intptr_t) nMaxMoves;
    ...
    if (gnubgid) {
        PyObject *tmp = Py_BuildValue("{s:s,s:N,s:N}", "hinttype", "chequer", "gnubgid", gnubgid, "hint", retval);
        Py_DECREF(retval);
        Py_DECREF(gnubgid);
        retval = tmp;
    }
...
static PyObject *
PythonUpdateUI(PyObject * self, PyObject * args)
{
    (void) self;
    (void) args;
    Py_RETURN_NONE;
...
PythonNavigate(PyObject * self, PyObject * args, PyObject * keywds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "|ii", (char **) kwlist, &nextRecord, &nextGame))
        return NULL;
...
/* Minimal PythonGame: info + game records (no analysis). For full match() output. */
static PyObject *PythonGame(const listOLD * plGame, int /* doAnalysis */, int /* verbose */,
                            void * /* scMatch */, int includeBoards, void * /* ms */)
{
    ...
    if (g->anScore[0] + 1 == (int) g->nMatch || g->anScore[1] + 1 == (int) g->nMatch)
    ...
    for (pl = pl->plNext; pl != plGame; pl = pl->plNext) {
        ...
        case MOVE_NORMAL:
            action = "move";
            player = pmr->fPlayer;
            DictSetItemSteal(recordDict, "dice", Py_BuildValue("(ii)", pmr->anDice[0], pmr->anDice[1]));
        ...
        case MOVE_SETBOARD:
            action = "set";
            DictSetItemSteal(recordDict, "board", PyUnicode_FromString(PositionIDFromKey(&pmr->sb.key)));
            if (includeBoards)
        ...
        case MOVE_SETCUBEPOS:
            action = "set";
            DictSetItemSteal(recordDict, "cube-owner", PyUnicode_FromString(pmr->scp.fCubeOwner == 0 ? "X" : (pmr->scp.fCubeOwner == 1 ? "O" : "centered")));
            break;
        default:
            break;
        }
 ...
 * Exposed as: gnubg.match(analysis=..., boards=..., statistics=..., verbose=...)
 */
static PyObject *
PythonMatch(PyObject * self, PyObject * args, PyObject * keywds)
{
    static const char *kwlist[] = { "analysis", "boards", "statistics", "verbose", NULL };
    int includeAnalysis = 1, verboseAnalysis = 0, statistics = 0, boards = 1;
    const listOLD *firstGame = lMatch.plNext ? (const listOLD*)lMatch.plNext->p : NULL;
    ...
    if (getFinalScore(anFinalScore)) {
        if (anFinalScore[0] > (int) g->nMatch) result = -1;
        else if (anFinalScore[1] > (int) g->nMatch) result = 1;
    }
    DictSetItemSteal(matchInfoDict, "result", PyLong_FromLong(result));
    {
        const char *v[] = { "Standard", "Nackgammon", "Hypergammon1", "Hypergammon2", "Hypergammon3" };
    ...
    }
    {
        ...
        if (n) {
            ...
            if (rules) {
                unsigned int idx = 0;
                if (!g->fCubeUse) PyTuple_SET_ITEM(rules, idx++, PyUnicode_FromString("NoCube"));
                if (g->fCrawford) PyTuple_SET_ITEM(rules, idx++, PyUnicode_FromString("Crawford"));
                if (g->fJacoby) PyTuple_SET_ITEM(rules, idx, PyUnicode_FromString("Jacoby"));
    ...
    nGames = 0;
    for (const listOLD *pl = lMatch.plNext; pl != &lMatch; pl = pl->plNext) {
        PyObject *pg = PythonGame((const listOLD*)pl->p, includeAnalysis, verboseAnalysis, statistics ? &scm : NULL, boards, NULL);
        if (!pg) {
    ...
    /* statistics/stats: PyGameStats lives in original gnubgmodule.c; omit for library build */
    (void) statistics;
    (void) scm;
    return matchDict;
}
 ...
 */
static PyObject *
PythonGetEvalHintFilter(PyObject * self, PyObject * args)
{
 ...
 */
static PyObject *
PythonSetEvalHintFilter(PyObject * self, PyObject * args)
{
    PyObject *pyMoveFilters = NULL;
    ...
    if (!PyArg_ParseTuple(args, "|O:setevalhintfilter", &pyMoveFilters))
        return NULL;
    if (!pyMoveFilters) {
        PyErr_SetString(PyExc_TypeError, "setevalhintfilter requires 1 argument: a list of move filters (see gnubg.getevalhintfilter() for an example)");
        return NULL;
    }
 ...
 */
static PyObject *
PythonShow(PyObject * self, PyObject * args)
{
    const char *pch = NULL;
    char *sz = NULL;
    PyObject *p = NULL;
 ...
 */
static PyObject *
PythonNextTurn(PyObject * self, PyObject * args)
{
 ...
 */
static PyObject *
PythonSetGNUbgID(PyObject * self, PyObject * args)
{
    const char *pch = NULL;
    char *sz = NULL;
...
static PyMethodDef GnubgMethods[] = {
    ...
    {"evaluate", PythonEvaluate, METH_VARARGS,
     ...
     "    returns: tuple of 6 floats (win, wingammon, winbackgammon, losegammon, losebackgammon, equity)"},
    ...
    {"findbestmove", PythonFindBestMove, METH_VARARGS,
     ...
     "    arguments: [board], [cubeinfo], [evalcontext], [dice], [movefilters] (dice required if no game)\n"
     "    returns: tuple of (from, to) pairs, 1-based"},

    {"met", PythonMET, METH_VARARGS,
     "Return match equity table\n"
     "    arguments: [max score] (optional)\n"
     "    returns: list of 3: pre-Crawford table, post-Crawford player 0, post-Crawford player 1"},

    {"matchid", PythonMatchID, METH_VARARGS,
     "Return match ID string\n"
     "    arguments: [cubeinfo], [posinfo] (optional; from gnubg.cubeinfo(), gnubg.posinfo())\n"
     "    returns: match ID string"},

    {"gnubgid", PythonGnubgID, METH_VARARGS,
     "Return GNUbgID string (positionid:matchid)\n"
     "    arguments: [board], [cubeinfo], [posinfo] (optional; use 0 or all 3)\n"
    ...
    {"navigate", (PyCFunction) (PyCFunctionWithKeywords) PythonNavigate, METH_VARARGS | METH_KEYWORDS,
     "Navigate match/session\n"
     "    arguments: next=N, game=N (optional)\n"
     "    returns: None or (records_moved, games_moved)"},

    {"match", (PyCFunction) (PyCFunctionWithKeywords) PythonMatch, METH_VARARGS | METH_KEYWORDS,
     "Get current match\n"
