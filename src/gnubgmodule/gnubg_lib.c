...
const char *intro_string =
    N_("This program comes with ABSOLUTELY NO WARRANTY; for details type `show "
       "warranty'.\n"
...
#define EVALSETUP_WORLDCLASS                                                   \
  {                                                                            \
    /* evaltype */                                                             \
    EVAL_EVAL, /* evalcontext */                                               \
        {.fCubeful = TRUE,                                                     \
         .nPlies = 2,                                                          \
         .fUsePrune = TRUE,                                                    \
         .fDeterministic = TRUE,                                               \
         .rNoise = 0.0f}, /* rolloutcontext */                                 \
    {                                                                          \
      {                                                                        \
          {.fCubeful = FALSE,                                                  \
           .nPlies = 2,                                                        \
           .fUsePrune = TRUE,                                                  \
           .fDeterministic = TRUE,                                             \
           .rNoise = 0.0f}, /* player 0 cube decision */                       \
          {.fCubeful = FALSE,                                                  \
           .nPlies = 2,                                                        \
           .fUsePrune = TRUE,                                                  \
           .fDeterministic = TRUE,                                             \
           .rNoise = 0.0f} /* player 1 cube decision */                        \
      },                                                                       \
          {                                                                    \
              {.fCubeful = FALSE,                                              \
               .nPlies = 0,                                                    \
               .fUsePrune = TRUE,                                              \
               .fDeterministic = TRUE,                                         \
               .rNoise = 0.0f}, /* player 0 chequerplay */                     \
              {.fCubeful = FALSE,                                              \
               .nPlies = 0,                                                    \
               .fUsePrune = TRUE,                                              \
               .fDeterministic = TRUE,                                         \
               .rNoise = 0.0f} /* player 1 chequerplay */                      \
          },                                                                   \
          {                                                                    \
              {.fCubeful = FALSE,                                              \
               .nPlies = 2,                                                    \
               .fUsePrune = TRUE,                                              \
               .fDeterministic = TRUE,                                         \
               .rNoise = 0.0f}, /* p 0 late cube decision */                   \
              {.fCubeful = FALSE,                                              \
               .nPlies = 2,                                                    \
               .fUsePrune = TRUE,                                              \
               .fDeterministic = TRUE,                                         \
               .rNoise = 0.0f} /* p 1 late cube decision */                    \
          },                                                                   \
          {                                                                    \
              {.fCubeful = FALSE,                                              \
               .nPlies = 0,                                                    \
               .fUsePrune = TRUE,                                              \
               .fDeterministic = TRUE,                                         \
               .rNoise = 0.0f}, /* p 0 late chequerplay */                     \
              {.fCubeful = FALSE,                                              \
               .nPlies = 0,                                                    \
               .fUsePrune = TRUE,                                              \
               .fDeterministic = TRUE,                                         \
               .rNoise = 0.0f} /* p 1 late chequerplay */                      \
          },                                                                   \
          {.fCubeful = FALSE,                                                  \
           .nPlies = 2,                                                        \
           .fUsePrune = TRUE,                                                  \
           .fDeterministic = TRUE,                                             \
           .rNoise = 0.0f}, /* truncate cube decision */                       \
          {.fCubeful = FALSE,                                                  \
           .nPlies = 2,                                                        \
           .fUsePrune = TRUE,                                                  \
           .fDeterministic = TRUE,                                             \
           .rNoise = 0.0f}, /* truncate chequerplay */                         \
          {MOVEFILTER_NORMAL, MOVEFILTER_NORMAL},                              \
          {MOVEFILTER_NORMAL, MOVEFILTER_NORMAL}, FALSE, /* cubeful */         \
          TRUE,         /* variance reduction */                               \
          FALSE,        /* initial position */                                 \
          TRUE,         /* rotate */                                           \
          TRUE,         /* truncate at BEAROFF2 for cubeless rollouts */       \
...
player ap[2] = {{"gnubg", PLAYER_GNU, EVALSETUP_WORLDCLASS,
                 EVALSETUP_WORLDCLASS, MOVEFILTER_NORMAL, 0, NULL},
                {"user", PLAYER_HUMAN, EVALSETUP_WORLDCLASS,
                 EVALSETUP_WORLDCLASS, MOVEFILTER_NORMAL, 0, NULL}};

char default_names[2][MAX_NAME_LEN] = {"gnubg", "user"};

/* Usage strings */
static char szDICE[] = N_("<die> <die>"), szCOMMAND[] = N_("<command>"),
            szCOMMENT[] = N_("<comment>"), szENDPOINT[] = N_("<host>:<port>"),
            szER[] = "evaluation|rollout", szFILENAME[] = N_("<filename>"),
            szKEYVALUE[] = N_("[<key>=<value> ...]"),
            szLENGTH[] = N_("<length>"), szLIMIT[] = N_("<limit>"),
            szMILLISECONDS[] = N_("<milliseconds>"),
            szMOVE[] = N_("<from> <to> ..."),
            szFILTER[] = N_("<ply> <num.xjoin to accept (0 = skip)> "
                            "[<num. of extra moves to accept> <tolerance>]"),
            szNAME[] = N_("<name>"), szQUIET[] = "[quiet]",
            szLANG[] = N_("system|<language code>"), szONOFF[] = "on|off",
            szOPTCOMMAND[] = N_("[command]"), szOPTDATE[] = N_("[yyyy-mm-dd]"),
            szOPTDEPTH[] = N_("[depth]"), szOPTFILENAME[] = N_("[filename]"),
            szOPTLENGTH[] = N_("[length]"),
            szOPTMODULUSOPTSEED[] =
                N_("[modulus <modulus>|factors <factor> <factor>] "
                   "[seed]"),
            szOPTNAME[] = N_("[name]"), szOPTPOSITION[] = N_("[position]"),
            szOPTSEED[] = N_("[seed]"), szOPTVALUE[] = N_("[value]"),
            szPLAYER[] = N_("<player>"),
            szPLAYEROPTRATING[] = N_("<player> [rating]"),
            szPLIES[] = N_("<plies>"), szPOSITION[] = N_("<position>"),
            szPRIORITY[] = N_("<priority>"), szPROMPT[] = N_("<prompt>"),
            szSCORE[] = N_("<score> [length]"), szSIZE[] = N_("<size>"),
            szSTEP[] = N_("[game|roll|rolled|marked] <count>"),
            szTRIALS[] = N_("<trials>"), szVALUE[] = N_("<value>"),
            szMATCHID[] = N_("<matchid>"), szGNUBGID[] = N_("<gnubgid>"),
            szXGID[] = N_("<xgid>"), szURL[] = "<URL>",
...
extern int ParsePosition(TanBoard an, char **ppch, char *pchDesc) {
  ...
  if (strlen(pch) == 20) { /* gnubg-nn position string */
    ...
    for (i = 0; i < 10; ++i) {
      if (pch[2 * i + 0] >= 'A' && pch[2 * i + 0] <= 'P' &&
          pch[2 * i + 1] >= 'A' && pch[2 * i + 1] <= 'P')
...
extern void PortableSignal(int nSignal, void (*p)(int), psighandler *pOld,
                           int fRestart) {
...
extern void HandleCommand(char *sz, command *ac) {
  ...
  if (ac == acTop &&
      (isdigit(*pch) || !StrNCaseCmp(pch, "bar/", cch > 4 ? 4 : cch))) {
    if (pch + cch < sz)
      pch[cch] = ' ';
...
extern void GetMatchStateCubeInfo(cubeinfo *pci, const matchstate *pms) {
  ...
  SetCubeInfo(pci, pms->nCube, pms->fCubeOwner, pms->fMove, pms->nMatchTo,
...
static void DisplayAnalysis(moverecord *pmr) {
  ...
  switch (pmr->mt) {
  case MOVE_NORMAL:
    DisplayCubeAnalysis(pmr->CubeDecPtr->aarOutput, pmr->CubeDecPtr->aarStdDev,
    ...
    for (i = 0; i < pmr->ml.cMoves; i++) {
      if (i >= 10 /* FIXME allow user to choose limit */ && i != pmr->n.iMove)
        continue;
      outputc(i == pmr->n.iMove ? '*' : ' ');
      output(FormatMoveHint(szBuf, &ms, &pmr->ml, i,
                            i != pmr->n.iMove || i != pmr->ml.cMoves - 1 ||
  ...
  case MOVE_SETDICE:
    if (pmr->rLuck != ERR_VAL)
      outputf("%s %u%u (%s):\n", _("Rolled"), pmr->anDice[0], pmr->anDice[1],
              GetLuckAnalysis(&ms, pmr->rLuck));
    break;
...
extern void ShowBoard(void) {
  char sz[50], szCube[50], szPlayer0[MAX_NAME_LEN + 3],
      szPlayer1[MAX_NAME_LEN + 3], szScore0[50], szScore1[50], szMatch[50];
  TanBoard an;
...
#if defined(USE_GTK)
    if (fX) {
      TanBoard anBoardTemp = {{0}};
      InitBoard(anBoardTemp, ms.bgv);
      game_set(BOARD(pwBoard), anBoardTemp, 0, ap[1].szName, ap[0].szName,
               ms.nMatchTo, ms.anScore[1], ms.anScore[0], 0, 0, FALSE,
               anChequers[ms.bgv]);
    } else
#endif
...
#endif
  {
    ...
    if (fOutputRawboard) {
      outputl(FIBSBoard(szBoard, an, ms.fMove, ap[1].szName, ap[0].szName,
                        ms.nMatchTo, ms.anScore[1], ms.anScore[0], ms.anDice[0],
                        ms.anDice[1], ms.nCube, ms.fCubeOwner, ms.fDoubled,
                        ms.fTurn, ms.fCrawford, anChequers[ms.bgv],
                        ms.fPostCrawford));
    ...
    sprintf(apch[1] = szScore0,
            ngettext("%d point", "%d points", ms.anScore[0]), ms.anScore[0]);

    sprintf(apch[5] = szScore1,
            ngettext("%d point", "%d points", ms.anScore[1]), ms.anScore[1]);
    ...
    if (ms.fDoubled) {
      ...
      if (ms.fCubeOwner < 0) {
        ...
        if (ms.nMatchTo)
          if (ms.nMatchTo == 1)
            sprintf(szCube,
                    ngettext("%d point match", "%d points match", ms.nMatchTo),
                    ms.nMatchTo);
            ...
            sprintf(szCube,
                    ngettext("%d point match (Cube: %d)",
                             "%d points match (Cube: %d)", ms.nMatchTo),
    ...
    outputl(DrawBoard(szBoard, (ConstTanBoard)an, ms.fMove, apch,
                      MatchIDFromMatchState(&ms), anChequers[ms.bgv]));
    ...
    game_set(BOARD(pwBoard), an, ms.fMove, ap[1].szName, ap[0].szName,
             ms.nMatchTo, ms.anScore[1], ms.anScore[0], ms.anDice[0],
             ms.anDice[1],
             ap[ms.fTurn].pt != PLAYER_HUMAN && !fComputing && !nNextTurn,
             anChequers[ms.bgv]);
  }
#endif
...
extern void CommandEval(char *sz) {
  ...
  if (ms.gs == GAME_NONE)
    ...
    SetCubeInfo(&ci, ms.nCube, ms.fCubeOwner, n ? !ms.fMove : ms.fMove,
                ms.nMatchTo, ms.anScore, ms.fCrawford, ms.fJacoby, nBeavers,
                ms.bgv);
  ...
  dd.n = n;
  if (RunAsyncProcess((AsyncFun)asyncDumpDecision, &dd,
                      _("Evaluating position...")) == 0) {
#if defined(USE_GTK)
    if (fX)
      GTKEval(szOutput);
    else
...
extern command *FindHelpCommand(command *pcBase, char *sz, char *pchCommand,
                                char *pchUsage) {
...
extern char *FormatMoveHint(char *sz, const matchstate *pms, movelist *pml,
  ...
  sprintf(strchr(sz, 0),
          "%-14s   %-28s %s: ", FormatEval(szTemp, &pml->amMoves[i].esMove),
          FormatMove(szMove, pms->anBoard, pml->amMoves[i].anMove),
  ...
  if (fDetailProb) {
    ...
    case EVAL_EVAL:
      ...
      break;
    case EVAL_ROLLOUT:
      strcat(sz, OutputRolloutResult(
                     "     ", NULL, (float(*)[NUM_ROLLOUT_OUTPUTS])ar,
                     (float(*)[NUM_ROLLOUT_OUTPUTS])arStdDev, &ci, 0, 1,
...
static void HintResigned(void) {
  ...
  dd.pci = &ci;
  dd.pec = &GetEvalCube()->ec;
  if (RunAsyncProcess((AsyncFun)asyncMoveDecisionE, &dd,
                      _("Considering resignation...")) != 0)
    return;
  ...
  if (!ms.nMatchTo || !fOutputMWC) {
    ...
    outputf("%s : %s\n\n", _("Correct resign decision"),
            (rEqBefore - rEqAfter >= 0) ? _("Accept") : _("Reject"));
    ...
    outputf("%s : %s\n\n", _("Correct resign decision"),
            (rEqAfter - rEqBefore >= 0) ? _("Accept") : _("Reject"));
  }
}
...
static int hint_cube(moverecord *pmr, cubeinfo *pci) {
  ...
  if (pmr->CubeDecPtr->esDouble.et == EVAL_NONE) {
    ...
    dd.pci = pci;
    dd.pes = GetEvalCube();
    if (RunAsyncProcess((AsyncFun)asyncCubeDecision, &dd,
...
extern void hint_double(int show, int did_double) {
  ...
  if (dt != DT_NORMAL) {
    if (show)
      outputerrf(_("This decision is part of beaver/raccoon sequence and "
                   "cannot be hinted"));
    return;
  }
...
#endif
  if (show)
    outputl(OutputCubeAnalysis(pmr->CubeDecPtr->aarOutput,
                               pmr->CubeDecPtr->aarStdDev,
                               &pmr->CubeDecPtr->esDouble, &ci, -1));
}
...
extern void hint_move(char *sz, gboolean show, procrecorddata *procdatarec) {
  ...
  if (pmr->esChequer.et == EVAL_NONE) {
    ...
    if (procdatarec) {
      show = FALSE;
      fShowProgress =
          (procdatarec->avInputData[PROCREC_HINT_ARGIN_SHOWPROGRESS] != NULL);
    }
    if ((RunAsyncProcess((AsyncFun)asyncFindMove, &fd,
                         _("Considering move...")) != 0) ||
        MT_SafeGet(&fInterrupt)) {
  ...
  for (i = 0; i < n; i++) {
    if (show)
      ...
      procdatarec->avOutputData[PROCREC_HINT_ARGOUT_INDEX] =
          (void *)(ptrdiff_t)i;
...
extern void CommandQuit(char *UNUSED(sz)) { PromptForExit(); }
...
extern void CommandRollout(char *sz) {
  ...
  if (CountTokens(sz) > 0) {
    outputerrf("%s", _("The rollout command takes no arguments and only "
                       "rollouts the current position"));
    return;
  }
  if (ms.gs != GAME_PLAYING) {
  ...
  SetCubeInfo(&ci, ms.nCube, ms.fCubeOwner, ms.fMove, ms.nMatchTo, ms.anScore,
              ms.fCrawford, ms.fJacoby, nBeavers, ms.bgv);
  RolloutProgressStart(&ci, 1, NULL, &rcRollout, asz, FALSE, &p);
  GeneralEvaluationR(arOutput, arStdDev, arsStatistics, (ConstTanBoard)anBoard,
                     &ci, &rcRollout, RolloutProgress, p);
...
static void LoadCommands(FILE *pf, char *szFile) {
  ...
  while (fgets(sz, sizeof(sz), pf) != NULL) {
    ...
    if (!strcmp(sz, ">")) {
      ...
      /* Ideally we should be able to handle both > print 1+1 sys.exit() and >
       * print 1+1 currently we only handle the latter... */
...
extern void CommandCopy(char *UNUSED(sz)) {
  ...
  char szOut[2048];
  char szCube[40], szPlayer0[MAX_NAME_LEN + 3], szPlayer1[MAX_NAME_LEN + 3],
  ...
  sprintf(aps[1] = szScore0, ngettext("%d point", "%d points", ms.anScore[0]),
          ms.anScore[0]);

  sprintf(aps[5] = szScore1, ngettext("%d point", "%d points", ms.anScore[1]),
          ms.anScore[1]);
  ...
  DrawBoard(szOut, (ConstTanBoard)anBoardTemp, ms.fMove, aps,
            MatchIDFromMatchState(&ms), anChequers[ms.bgv]);
...
static void SaveRNGSettings(FILE *pf, const char *sz, rng rngCurrent,
                            rngcontext *rngctx) {
...
static void
SaveMoveFilterSettings(FILE *pf, const char *sz,
  ...
  for (i = 0; i < MAX_FILTER_PLIES; ++i)
    for (j = 0; j <= i; ++j) {
      fprintf(pf, "%s %d  %d  %d %d %s\n", sz, i + 1, j, aamf[i][j].Accept,
              aamf[i][j].Extra,
              g_ascii_formatd(buf, G_ASCII_DTOSTR_BUF_SIZE, "%0.3g",
                              aamf[i][j].Threshold));
    }
}

static void SaveEvalSettings(FILE *pf, const char *sz, evalcontext *pec) {

  gchar buf[G_ASCII_DTOSTR_BUF_SIZE];
  gchar *szNoise =
      g_ascii_formatd(buf, G_ASCII_DTOSTR_BUF_SIZE, "%0.3f", pec->rNoise);
  fprintf(pf,
          "%s plies %u\n"
          "%s prune %s\n"
          "%s cubeful %s\n"
          "%s noise %s\n"
          "%s deterministic %s\n",
          sz, pec->nPlies, sz, pec->fUsePrune ? "on" : "off", sz,
          pec->fCubeful ? "on" : "off", sz, szNoise, sz,
          pec->fDeterministic ? "on" : "off");
}
...
extern void SaveRolloutSettings(FILE *pf, const char *sz, rolloutcontext *prc) {
  ...
  fprintf(pf,
          ...
          "%s jsd limit %s\n",
          sz, prc->fCubeful ? "on" : "off", sz, prc->fVarRedn ? "on" : "off",
          sz, prc->fRotate ? "on" : "off", sz, prc->fInitial ? "on" : "off", sz,
          prc->fDoTruncate ? "on" : "off", sz, prc->nTruncate, sz,
          prc->fTruncBearoff2 ? "on" : "off", sz,
          prc->fTruncBearoffOS ? "on" : "off", sz,
          prc->fLateEvals ? "on" : "off", sz, prc->nLate, sz, prc->nTrials, sz,
          fCubeEqualChequer ? "on" : "off", sz, fPlayersAreSame ? "on" : "off",
          sz, fTruncEqualPlayer0 ? "on" : "off", sz,
          prc->fStopOnSTD ? "on" : "off", sz, prc->nMinimumGames, sz, szTemp1,
          sz, prc->fStopOnJsd ? "on" : "off", sz, prc->nMinimumJsdGames, sz,
          szTemp2);
...
static void SaveAnalysisSettings(FILE *pf) {
  ...
  g_ascii_formatd(aszThr[0], G_ASCII_DTOSTR_BUF_SIZE, "%0.3f",
                  arSkillLevel[SKILL_BAD]);
  g_ascii_formatd(aszThr[1], G_ASCII_DTOSTR_BUF_SIZE, "%0.3f",
                  arSkillLevel[SKILL_DOUBTFUL]);
  g_ascii_formatd(aszThr[2], G_ASCII_DTOSTR_BUF_SIZE, "%0.3f",
                  arLuckLevel[LUCK_GOOD]);
  g_ascii_formatd(aszThr[3], G_ASCII_DTOSTR_BUF_SIZE, "%0.3f",
                  arLuckLevel[LUCK_BAD]);
  g_ascii_formatd(aszThr[4], G_ASCII_DTOSTR_BUF_SIZE, "%0.3f",
                  arSkillLevel[SKILL_VERYBAD]);
  g_ascii_formatd(aszThr[5], G_ASCII_DTOSTR_BUF_SIZE, "%0.3f",
                  arLuckLevel[LUCK_VERYGOOD]);
  g_ascii_formatd(aszThr[6], G_ASCII_DTOSTR_BUF_SIZE, "%0.3f",
...
static void SaveScoreMapSettings(FILE *pf) {
...
static void SaveMiscSettings(FILE *pf) {
  ...
  fprintf(
      pf, "set output errorratefactor %s\n",
    ...
    while (pc && pc->sz) {
      if (!StrNCaseCmp(pchCurrent, pc->sz, strlen(pchCurrent))) {
        ...
        if (pc == acSetPlayer || pc == acSetRolloutPlayer ||
            pc == acSetRolloutLatePlayer || pc == acSetAnalysisPlayer ||
            pc == acSetCheatPlayer) {
          pcResume = pc;
          pc = &cPlayerBoth;
        }
...
static char **CompleteKeyword(const char *szText, int iStart,
                              int UNUSED(iEnd)) {
...
#if defined(HAVE_LIB_READLINE)
static char *locale_from_utf8(const char *sz) {
  char *ret;
  GError *error = NULL;
  g_assert(sz);
...
static char *locale_to_utf8(const char *sz) {
  char *ret;
  GError *error = NULL;
  g_assert(sz);
...
extern void CommandSetOutputOutput(char *sz) {
  SetToggle("output", &foutput_on, sz, _("output will be shown"),
            _("output will not be shown"));
  return;
}
...
static void move_rc_files(void) {
...
static int CreateGnubgDirectory(void) {
  char *newfile;
...
static void setup_readline(void) {
  ...
  /* libedit (macOS) does not provide rl_filename_quote_characters */
  rl_filename_quote_characters = szCommandSeparators;
#endif
  rl_completer_quote_characters = "\"";
  /* assume readline 4.2 or later; libedit has incompatible type for this */
#if !defined(__APPLE__)
...
#if !defined(USE_GTK)
static void PushSplash(char *UNUSED(unused), char *UNUSED(heading),
                       char *UNUSED(message)) {}
#endif
...
static void init_rng(void) {
  ...
  if (!(rngctxRollout =
            InitRNG(&rcRollout.nSeed, NULL, TRUE, rcRollout.rngRollout))) {
...
static void null_debug(const gchar *UNUSED(dom),
                       GLogLevelFlags UNUSED(logflags),
                       const gchar *UNUSED(message), gpointer UNUSED(unused)) {}

static char *pchPythonScript = NULL;
static int fPython = FALSE;
static int fNoX = FALSE;

static gboolean callback_parse_python_option(const gchar *UNUSED(name),
                                             const gchar *value,
...
int gnubg_lib_unused_main(int argc, char *argv[]) {
  ...
  static int fNoBearoff = FALSE, fSplash = FALSE, fNoTTY = FALSE,
             show_version = FALSE, debug = FALSE;
  GOptionEntry ao[] = {
      ...
      {"lang", 'l', 0, G_OPTION_ARG_STRING, &lang, N_("Set language to LANG"),
       "LANG"},
      {"python", 'p', G_OPTION_FLAG_OPTIONAL_ARG, G_OPTION_ARG_CALLBACK,
       callback_parse_python_option,
       N_("Start in Python mode or evaluate code in FILE and exit. Implies "
          "--tty if used with a FILE argument."),
       "FILE"},
      {"quiet", 'q', 0, G_OPTION_ARG_NONE, &fQuiet, N_("Disable sound effects"),
       NULL},
      {"no-rc", 'r', 0, G_OPTION_ARG_NONE, &fNoRC,
       N_("Do not read .gnubgrc and .gnubgautorc commands"), NULL},
      {"splash", 'S', 0, G_OPTION_ARG_NONE, &fSplash,
       N_("Show gtk splash screen"), NULL},
      {"tty", 't', 0, G_OPTION_ARG_NONE, &fNoX,
       N_("Start the command-line instead of using the graphical interface"),
       NULL},
      ...
      {"debug", 'd', 0, G_OPTION_ARG_NONE, &debug, N_("Turn on debug"), NULL},
...
extern void CommandEq2MWC(char *sz) {
  ...
  outputf("%s = %+6.3f: %6.2f%%\n", _("MWC for equity"), -1.0,
          100.0f * eq2mwc(-1.0, &ci));
  outputf("%s = %+6.3f: %6.2f%%\n", _("MWC for equity"), +1.0,
          100.0f * eq2mwc(+1.0, &ci));
  outputf("%s:\n", _("By linear interpolation"));
  outputf("%s = %+6.3f: %6.2f%%\n", _("MWC for equity"), rEq,
          100.0f * eq2mwc(rEq, &ci));
}
...
extern void CommandMWC2Eq(char *sz) {
  ...
  outputf("%s = %6.2f%%: %+6.3f\n", _("Equity for MWC"),
          100.0f * eq2mwc(-1.0, &ci), -1.0);
  outputf("%s = %6.2f%%: %+6.3f\n", _("Equity for MWC"),
          100.0f * eq2mwc(+1.0, &ci), +1.0);
  outputf("%s:\n", _("By linear interpolation"));
  outputf("%s = %6.2f%%: %+6.3f\n", _("Equity for MWC"), 100.0f * rMwc,
          mwc2eq(rMwc, &ci));
}
...
extern int DeleteKeyName(const char sz[]) {
  ...
  for (int i = 0; i < keyNamesFirstEmpty; i++) {
    if (!strcmp(sz, keyNames[i])) {
      // g_message("EXISTS! %s=%s, i=%d, keyNamesFirstEmpty=%d",
      // sz,keyNames[i],i,keyNamesFirstEmpty);
...
extern int AddKeyName(const char sz[]) {
...
extern void CommandSwapPlayers(char *UNUSED(sz)) {
  ...
  /* VERSION1: if fUseKeyNames enabled, then add the new player1 to the key
 players (now still player0) VERSION2: also add if fUseKeyNames is not enabled
 yet, so users don't think they need to add names manually. We only check that
 the permutation wasn't launched by the SmartSit() function, which would mean
...
#if defined(HAVE_LIB_READLINE)
extern void CommandHistory(char *UNUSED(sz)) {
  int i;
...
extern int EPC(const TanBoard anBoard, float *arEPC, float *arMu,
  ...
  const float x = (2 * 3 + 3 * 4 + 4 * 5 + 4 * 6 + 6 * 7 + 5 * 8 + 4 * 9 +
                   2 * 10 + 2 * 11 + 1 * 12 + 1 * 16 + 1 * 20 + 1 * 24) /
                  36.0f;
  ...
  if (isBearoff(pbc1, anBoard)) {
    ...
    for (i = 0; i < 2; ++i) {
      unsigned int n =
          PositionBearoff(anBoard[i], pbc1->nPoints, pbc1->nChequers);
    ...
    for (i = 0; i < 2; ++i) {
      unsigned int n =
          PositionBearoff(anBoard[i], pbcOS->nPoints, pbcOS->nChequers);
...
char *SetupLanguage(const char *newLangCode) {
  ...
  if (!newLangCode || !strcmp(newLangCode, "system") ||
      !strcmp(newLangCode, ""))
...
void asyncDumpDecision(decisionData *pdd) {
  if (DumpPosition(pdd->pboard, pdd->szOutput, pdd->pec, pdd->pci, fOutputMWC,
                   fOutputWinPC, pdd->n, MatchIDFromMatchState(&ms)) != 0)
    MT_SetResultFailed();
}
...
void asyncEvalRoll(decisionData *pdd) {
  EvaluateRoll(pdd->aarOutput[0], ms.anDice[0], ms.anDice[1], pdd->pboard,
               pdd->pci, pdd->pec);
}

void asyncAnalyzeMove(moveData *pmd) {
  if (AnalyzeMove(pmd->pmr, pmd->pms, plGame, NULL, pmd->pesChequer,
                  pmd->pesCube, pmd->aamf, NULL, NULL) < 0)
    MT_SetResultFailed();
}

void asyncGammonRates(decisionData *pdd) {
  if (getCurrentGammonRates(pdd->aarRates, pdd->aarOutput[0], pdd->pboard,
                            pdd->pci, pdd->pec) < 0)
    MT_SetResultFailed();
}

void asyncMoveDecisionE(decisionData *pdd) {
  if (GeneralEvaluationE(pdd->aarOutput[0], pdd->pboard, pdd->pci, pdd->pec) <
      0)
    MT_SetResultFailed();
}

void asyncCubeDecisionE(decisionData *pdd) {
  if (GeneralCubeDecisionE(pdd->aarOutput, pdd->pboard, pdd->pci, pdd->pec,
                           pdd->pes) < 0)
    MT_SetResultFailed();
}
...
extern int RunAsyncProcess(AsyncFun fun, void *data, const char *msg) {
  int ret;
#if defined(USE_MULTITHREAD)
...
extern int get_input_discard(void) {
  ...
  if (autosave && fAutoSaveConfirmDelete) {
    if (!GetInputYN(_("Are you sure you want to discard the current match and "
  ...
  if (ms.gs == GAME_PLAYING && fConfirmNew)
    return GetInputYN(
        _("Are you sure you want to discard the current match? "));
